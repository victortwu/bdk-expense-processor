    ...
    try {
      ...
      if (expense.status !== 'ready') {
        results.push({
          documentId,
          success: false,
          error: `Cannot submit expense with status: ${expense.status}`,
        })
        failed++
        continue
      }
      ...
      try {
        ...
        await ddbClient.send(
          new UpdateCommand({
            ...
            Key: { pk: `DOC#${documentId}`, sk: 'state' },
            UpdateExpression:
              'SET #status = :status, qboDocNumber = :docNumber, qboPurchaseId = :purchaseId, submittedAt = :now, lastAttempt = :now, attempts = attempts + :inc, updatedAt = :now',
        ...
        await ddbClient.send(
          new UpdateCommand({
            ...
            Key: { pk: `DOC#${documentId}`, sk: 'state' },
            UpdateExpression:
              'SET #status = :status, failedAt = :now, failureReason = :reason, lastAttempt = :now, attempts = attempts + :inc, updatedAt = :now',
            ExpressionAttributeNames: { '#status': 'status' },
            ExpressionAttributeValues: {
              ':status': 'failed',
              ':now': now,
      ...
      console.error(`Error processing expense ${documentId}:`, err)
      results.push({
        documentId,
        success: false,
